...
class ReadingBase(BaseModel):
    ...
    @classmethod
    def handle_timestamps(cls, data: Any) -> Any:
        if isinstance(data, dict):
            # If the device sends its own timestamp (which is not a real datetime),
            # store it as device_timestamp and remove it so timestamp gets generated by default_factory (server time).
            if "timestamp" in data:
                # Only move it if it's the raw string from ESP32 or not a datetime object
                if isinstance(data["timestamp"], str) and "Z" not in data["timestamp"] and "T" not in data["timestamp"]:
                    data["device_timestamp"] = data.pop("timestamp")
                elif isinstance(data["timestamp"], (int, float)):
                    data["device_timestamp"] = str(data.pop("timestamp"))
        return data
...
class HistoryResponse(BaseModel):
    ...
    ldr_left: Optional[int] = Field(None, description="Left LDR raw sensor value")
    ldr_right: Optional[int] = Field(None, description="Right LDR raw sensor value")
    fan_status: Optional[str] = Field(None, description="Current status of the cooling fan")
    status: Optional[str] = Field(None, description="Overall device operational status")
